...
function sampleNPC.onTickEndNPC(v)
	...
	v.animationTimer = 1
	v.friendly = true --So that shots pass through eachother instead of getting stuck
	...
	if v.collidesBlockBottom or v.collidesBlockLeft or v.collidesBlockRight or v.collidesBlockUp then
		...
		for _,z in ipairs(Block.getIntersecting(v.x, v.y, v.x + v.width, v.y + v.height)) do
			if Colliders.collide(z, v) and table.icontains(Block.SLOPE, z.id) and not table.icontains(Block.SEMISOLID, z.id) and z.isHidden == false then
				v:kill(HARM_TYPE_OFFSCREEN)
			end
		end
	end
