...
local enemydamage = Misc.resolveFile("mmenemydamage.wav")
...
function sampleNPC.onTickNPC(v)
	...
	if not data.initialized then
		...
		v.ai1 = sampleNPCSettings.health
	end
	...
	if (v.animationFrame == 3 or v.animationFrame == 8) and v.animationTimer == 1 then
		bomb = NPC.spawn(854, v.x + (v.width / 4), v.y)
		local bombxspeed = vector.v2(player.x + 0.5 * player.width - (bomb.x + 0.5 * v.width))
		bomb.direction = v.direction
		bomb.speedY = -12
		bomb.speedX = bombxspeed.x / 100
	end
