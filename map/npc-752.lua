...
smwMap.setObjSettings(npcID,{
    ...
    onInitObj = (function(v)
        ...
        local padding = smwMap.pathSettings.cullingPadding
        ...
        for distance = 0, pathObj.splineLength, 8 do
            ...
            pathObj.minX = math.min(position.x - padding,pathObj.minX)
            pathObj.minY = math.min(position.y - padding,pathObj.minY)
            pathObj.maxX = math.max(position.x + padding,pathObj.maxX)
            pathObj.maxY = math.max(position.y + padding,pathObj.maxY)
        end
