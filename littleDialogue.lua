...
do
    ...
    local extraSettingsList = {
        "font","speakerNameFont","boxImage","continueArrowImage","scrollArrowImage","selectorImage","speakerNameBoxImage",
...
function boxInstanceFunctions:addTextEvent(pageIndex,addPos,eventName,eventArgs)
    if type(addPos) == "TextplusLayout" then -- addPos is a layout: find how long the layout is.
        local lineCount = #addPos
        local characterCount = 0

        -- Go through each line
        for lineIndex,line in ipairs(addPos) do
        ...
        addPos = characterCount
    elseif type(addPos) == "table" then -- addPos is a list of segments: find how long it is
        local characterCount = 0

        for _,segment in ipairs(addPos) do
            if segment.img ~= nil then
                characterCount = characterCount + 1
            else
...
function boxInstanceFunctions:activateTextEvents(startPos,endPos)
    ...
    -- The reason why this isn't done in the events themselves is that that can cause stack overlow and other nonsense
...
local mainBuffer = Graphics.CaptureBuffer(600,600)
local fullBuffer = Graphics.CaptureBuffer(600,600)
